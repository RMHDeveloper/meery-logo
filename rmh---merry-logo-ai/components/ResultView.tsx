...
const ResultView: React.FC<ResultViewProps> = ({ result, companyName, onRestart }) => {
  ...
  const fileName = `ornament-brief_${result.timestamp}.txt`;
  ...
  const handleWhatsAppShare = () => {
    const intro = `Design brief for our ${companyName || 'brand'} ornament, crafted with Merry Logo AI 🎄✨\n\n`;
    const text = encodeURIComponent(intro + result.description);
  ...
  return (
    <div className="space-y-6 animate-in fade-in zoom-in duration-500">
      <div className="bg-white p-6 rounded-2xl shadow-2xl border border-gray-100">
        <p className="text-gray-700 whitespace-pre-wrap leading-relaxed text-left">{result.description}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <button
          ...
          className="flex items-center justify-center space-x-2 bg-white border-2 border-[#2D5016] text-[#2D5016] hover:bg-[#2D5016] hover:text-white py-3 px-6 rounded-lg transition-all shadow-lg active:scale-95"
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
          </svg>
          <span className="font-semibold">Download as Text</span>
        </button>
      </div>
      ...
      <button
        onClick={onRestart}
        className="w-full mt-4 bg-transparent border-2 border-[#D4AF37] text-[#D4AF37] hover:bg-[#D4AF37] hover:text-white font-bold py-3 rounded-lg transition-all"
      >
